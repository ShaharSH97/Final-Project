...
clc; clear; close all;
%% =========================================================
% BASE DIRECTORY & AESTHETIC SETTINGS
%% =========================================================
set(groot,'defaultFigureColor','w');
set(groot,'defaultAxesColor','w'); 

% --- LaTeX Report Aesthetic Settings ---
...
grid(axClustersR,'off'); 

figClustersY = figure('Name','Yellow clusters (10s)');
axClustersY = axes(figClustersY);
hold(axClustersY,'on');
axis(axClustersY,'equal');
grid(axClustersY,'off'); 

figClustersB = figure('Name','Blue clusters (10s)');
axClustersB = axes(figClustersB);
hold(axClustersB,'on');
axis(axClustersB,'equal');
grid(axClustersB,'off'); 

% --- ORIENTATION ANGLE FIGURE ---
figOrient = figure('Name','Orientation phi(t) (10s)', 'Position',[250 250 800 500]);
axPhi = axes(figOrient);
hold(axPhi,'on');
grid(axPhi,'off'); 
xlabel(axPhi, '$t$ [s]', 'Interpreter', 'latex');
ylabel(axPhi, '$\phi$ [rad]', 'Interpreter', 'latex');
title(axPhi, 'Orientation Angle as a Function of Time', 'Interpreter', 'latex');
...
% הוספת המקרא במיקום אידיאלי
legend(axPhi, 'Location', 'best', 'Interpreter', 'latex');
...
title( ...
    ...
    'Interpreter','latex');
legend(axMeanOmegaAll,'Location','northwest','Interpreter','latex'); 
...
% Legend on the left & Y axis ticks jumps of 0.4
legend(axFit,'Location','west','Interpreter','latex');
yticks(axFit, -10:0.4:10); 
...
% Legend on the left & Y axis ticks jumps of 0.4
legend(axClean,'Location','west','Interpreter','latex');
yticks(axClean, -10:0.4:10); 
